...
export function VideoFormatSelector() {
  ...
  const handleFormatChange = (newFormat: VideoFormat) => {
    ...
    // Alertar sobre MP4 no Windows
    if (newFormat === "mp4" && isWindows) {
      const userConfirmed = window.confirm(
        "⚠️ AVISO: MP4 no Windows pode causar travamentos durante a gravação.\n\n" +
          "O sistema aplicará configurações muito conservadoras (baixo bitrate, resolução reduzida, chunks maiores) para tentar evitar problemas.\n\n" +
          "RECOMENDAÇÃO: Use WebM para melhor estabilidade no Windows.\n\n" +
          "Deseja continuar com MP4 mesmo assim?",
      );

      if (!userConfirmed) {
        // Se o usuário cancelou, manter o formato atual
        return;
      }
    }
