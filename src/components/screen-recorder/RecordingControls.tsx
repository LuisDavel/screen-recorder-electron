...
export function RecordingControls({
	...
	const handlePauseRecording = () => {
		try {
			...
			// CORREÇÃO: Informar usuário sobre otimizações para evitar travamento
			setTimeout(() => {
				showInfo(
					"💡 Dica: Evite pausas muito longas para prevenir travamentos no vídeo final",
				);
			}, 2000);
		} catch (error) {
			console.error("Erro ao pausar gravação:", error);
			showError("Erro ao pausar gravação");
		}
	};
	...
	const handleResumeRecording = () => {
		try {
			...
			// CORREÇÃO: Monitorar por problemas após retomar
			setTimeout(() => {
				const status = recorder.getStatus();
				if (status.recordedChunks > 0) {
					showSuccess(
						"✅ Gravação retomada com sucesso - monitorando qualidade",
					);
				}
			}, 1000);
		} catch (error) {
			console.error("Erro ao retomar gravação:", error);
			showError("Erro ao retomar gravação");
		}
	};
				...
				<div className="flex flex-col gap-2">
					<div className="flex items-center justify-between">
						<p className="text-muted-foreground text-sm">
							...
							{isRecording &&
								isPaused &&
								(isCameraOnlyMode ? "Câmera pausada" : "Gravação pausada")}
						</p>

						{isRecording && (
							<div className="flex items-center space-x-2">
								<div
									className={`h-3 w-3 rounded-full ${isPaused ? "bg-yellow-500" : "animate-pulse bg-red-500"}`}
								/>
