...
export class AdvancedScreenRecorderManager {
  ...
  private async setupVideoComposer(
    ...
    const shouldVideoComposerHandleCamera =
      cameraStream &&
      options.includeCameraOverlay &&
      !(options.includeFooter && options.footerConfig?.isEnabled);
    ...
    if (shouldVideoComposerHandleCamera) {
      ...
      options.footerConfig?.isEnabled
    ) {
      console.log(
        "🎥 FooterComposer will handle camera overlay (VideoComposer skipped)",
      );
    }
        ...
        if (videoFormatState.format === "mp4") {
          ...
          if (isWindows) {
            if (event.data.size < 20000) {
              ...
              console.warn(
                "⚠️ Performance baixa detectada com MP4 no Windows",
                {
                  chunkSize: event.data.size,
                  formato: "MP4",
                  plataforma: "Windows",
  ...
  private async processRecording(): Promise<void> {
    ...
    try {
      ...
      if (result.success) {
        ...
        console.log(
          "🎬 Verificando se videoConcatAPI está disponível:",
          !!window.videoConcatAPI,
        );

        try {
          // Disparar evento para concatenação automática
          const event = new CustomEvent(
            "video-concat:start-auto-concatenation",
            {
              detail: {
        ...
        // REMOVIDO: Upload automático do vídeo NÃO CONCATENADO
        // O upload agora acontece APENAS após a concatenação (VideoConcatenationOverlay)
        // para garantir que o vídeo enviado contenha intro + gravação + encerramento
        console.log(
          "⏭️ Upload será feito após concatenação com vídeos de intro/encerramento",
        );

        // Manter apenas log de configuração S3 para debug
        const s3Config = useS3ConfigStore.getState().config;
        if (s3Config.isEnabled && s3Config.isConfigured) {
          console.log(
            "✅ S3 configurado - Upload será automático após concatenação",
          );
        } else if (s3Config.isEnabled && !s3Config.isConfigured) {
          console.warn(
