...
import {
  VideoIntroManager,
  HOSPITAL_INTRO_MAPPING,
} from "../../video-intro-manager";
  ...
  ipcMain.handle(
    ...
    async (
      ...
      try {
        ...
        console.log(
          "🎬 DEBUG: Função findSystemFFmpeg existe:",
          typeof findSystemFFmpeg,
        );
        ...
        if (!ffmpegPath) {
          console.error(
            "❌ FFmpeg não encontrado em nenhum caminho do sistema",
          );
          return {
            success: false,
        ...
        if (!existsSync(forcedFFmpegPath)) {
          console.error(
            "❌ DEBUG: Caminho forçado não existe:",
            forcedFFmpegPath,
          );
          return {
            success: false,
            message:
              "FFmpeg não encontrado no caminho esperado: " + forcedFFmpegPath,
          };
        }
        ...
        const ffmpegArgs = [
          "-i",
          fullIntroPath, // Input 1: vídeo de introdução
          "-i",
          recordedVideoPath, // Input 2: vídeo gravado
          ...
          "-map",
          "[outv]", // Mapear vídeo de saída
          "-map",
          "[outa]", // Mapear áudio de saída
          "-c:v",
          "libx264", // Codec de vídeo H.264
          "-c:a",
          "aac", // Codec de áudio AAC
          "-b:a",
          "192k", // Bitrate de áudio para melhor qualidade
          "-movflags",
          "+faststart", // Otimização para streaming
          "-preset",
          "medium", // Preset de qualidade/velocidade
          "-crf",
          "18", // Alta qualidade visual (0-51, menor = melhor)
        ...
        console.log(
          "🔍 Verificando se arquivo de saída foi criado:",
          outputPath,
        );
