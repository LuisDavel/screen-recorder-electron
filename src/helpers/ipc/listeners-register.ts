...
import { addS3VideosEventListeners } from "./s3-videos/s3-videos-listeners";
...
import { registerFastVideoConcatListeners } from "./video-concat/video-concat-fast";
import { registerSimpleVideoConcatListeners } from "./video-concat/video-concat-simple";
...
export default function registerListeners(mainWindow: BrowserWindow) {
  ...
  try {
    ...
    try {
      ...
      console.log("✅ Função S3 upload chamada com sucesso");
    } catch (error) {
      console.error("❌ Erro ao chamar função S3 upload:", error);
      throw error;
    }

    // Registrar listeners de S3 videos
    console.log("🔧 Registrando listeners S3 videos...");
    try {
      addS3VideosEventListeners(mainWindow);
      console.log("✅ Listeners S3 videos registrados com sucesso");
    } catch (error) {
      console.error("❌ Erro ao registrar listeners S3 videos:", error);
      throw error;
    }
    ...
    console.log(
      "🔍 Tipo da função registerVideoConcatListeners:",
      typeof registerVideoConcatListeners,
    );
    ...
    try {
      ...
      // Registrar também os listeners OTIMIZADOS
      registerFastVideoConcatListeners(mainWindow);
      console.log(
        "🚀 Listeners OTIMIZADOS de concatenação registrados com sucesso",
      );

      // Registrar listeners SIMPLES (mais confiáveis)
      registerSimpleVideoConcatListeners(mainWindow);
      console.log(
        "🔧 Listeners SIMPLES de concatenação registrados com sucesso",
      );
    } catch (error) {
