...
export async function checkFFmpegAvailability(): Promise<FFmpegInfo> {
    try {
        ...
        if (!window.electronAPI) {
            ...
            return {
                isAvailable: false,
                error: "ElectronAPI não está disponível. Verifique se o preload está funcionando."
            };
        }

        console.log("🔍 Chamando FFmpeg check via IPC...");
        const result = await window.electronAPI.invoke(FFMPEG_CHECK_AVAILABILITY_CHANNEL);
...
export async function suggestFFmpegInstallation(): Promise<{
    ...
    try {
        ...
        return await window.electronAPI.invoke(FFMPEG_GET_INSTALL_SUGGESTION_CHANNEL);
    } catch (error) {
        return {
            canAutoInstall: false,
