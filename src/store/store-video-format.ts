	...
	persist(
		(set, get) => ({
			...
			getQualityBasedSettings: () => {
				...
				// CORREÇÃO: Reduzir chunkSize para minimizar travamento durante pause/resume
				let bitrate: number;
				let chunkSize: number;
				let frameRate: number;

				if (isMP4 && isWindows) {
					// MP4 no Windows - configurações mais conservadoras com chunks menores
					switch (quality) {
						case "low":
							bitrate = 800000; // 800 Kbps
							chunkSize = 8000; // 8 segundos (reduzido de 15s)
							frameRate = 12;
							break;
						case "medium":
							bitrate = 1200000; // 1.2 Mbps
							chunkSize = 6000; // 6 segundos (reduzido de 12s)
							frameRate = 15;
							break;
						case "high":
							bitrate = 1800000; // 1.8 Mbps
							chunkSize = 4000; // 4 segundos (reduzido de 10s)
							frameRate = 18;
							break;
					}
				} else if (isMP4) {
					// MP4 em outras plataformas - chunks menores para melhor estabilidade
					switch (quality) {
						case "low":
							bitrate = 1200000; // 1.2 Mbps
							chunkSize = 4000; // 4 segundos (reduzido de 8s)
							frameRate = 20;
							break;
						case "medium":
							bitrate = 2000000; // 2 Mbps
							chunkSize = 3000; // 3 segundos (reduzido de 6s)
							frameRate = 24;
							break;
						case "high":
							bitrate = 3000000; // 3 Mbps
							chunkSize = 2000; // 2 segundos (reduzido de 5s)
							frameRate = 30;
							break;
					}
				} else {
